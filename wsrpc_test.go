...
import (
	...
	"github.com/v-grabko1999/ws-rpc/jsonmsg"
)
...
func makeRegistry() *wsrpc.Registry {
	r := wsrpc.NewRegistry()
...
func TestServerSimple(t *testing.T) {
	...
	server := wsrpc.NewEndpoint(jsonmsg.NewCodec(s), registry)
...
func TestClient(t *testing.T) {
	...
	client := wsrpc.NewEndpoint(jsonmsg.NewCodec(c), nil)
...
func (e *EndpointPeer) Poke(request *nothing, reply *nothing, endpoint *wsrpc.Endpoint) error {
	if e.seen != nil {
...
func TestServerEndpointArg(t *testing.T) {
	...
	if peer.seen == nil {
		t.Fatalf("peer never saw a wsrpc.Endpoint")
	}
}
...
var _ wsrpc.FillArgser = BadFillArgsCodec{}
...
func TestFillArgsError(t *testing.T) {
	...
	server := wsrpc.NewEndpoint(codec, registry)
	server_err := make(chan error)
	go func() {
		server_err <- server.Serve()
	}()
...
func TestRegisterBadTooFewArguments(t *testing.T) {
	...
	testPanic(
		...
		"wsrpc.RegisterService: method wsrpc_test.TooFewArguments.TooFew is missing request/reply arguments",
	)
}
...
func TestRegisterBadNonPointerReply(t *testing.T) {
	...
	testPanic(
		...
		"wsrpc.RegisterService: method wsrpc_test.NonPointerReply.NonPointer reply argument must be a pointer type",
	)
}
...
func TestRegisterBadNoReturn(t *testing.T) {
	...
	testPanic(
		...
		"wsrpc.RegisterService: method wsrpc_test.NoReturn.NoReturn must return error",
	)
}
...
func TestRegisterBadNonErrorReturn(t *testing.T) {
	...
	testPanic(
		...
		"wsrpc.RegisterService: method wsrpc_test.NonErrorReturn.NonErrorReturn must return error",
	)
}
...
func TestRegisterBadMultiReturn(t *testing.T) {
	...
	testPanic(
		...
		"wsrpc.RegisterService: method wsrpc_test.MultiReturn.MultiReturn must return error",
	)
}
